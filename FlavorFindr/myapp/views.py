    ...
    if request.method == 'GET':
        ...
        if search_value:
            if search_operation == 'eq':
                filtered_items = [item for item in items if str(item.get(search_field, '')).lower() == search_value.lower()]
            elif search_operation in ['gt', 'lt'] and search_field in ['calories', 'protein', 'total_fat', 'carbohydrates']:
                search_value = float(search_value)  # Convert to float for numerical comparison
                if search_operation == 'gt':
                    filtered_items = [item for item in items if float(item.get(search_field, 0)) > search_value]
        ...
        if order_by:
            ...
            if order_by in ['calories', 'protein', 'total_fat', 'carbohydrates']:
                ...
                filtered_items.sort(key=lambda x: float(x.get(order_by, 0)), reverse=reverse_order)
            else:
                # Sort by string values (e.g., restaurant_name, item_name)
                filtered_items.sort(key=lambda x: str(x.get(order_by, '')).lower(), reverse=reverse_order)
...
def my_filter_function(items, get_data):
    ...
    if value:
        ...
        if attribute in ['restaurant_name', 'name']:
            if operation == 'eq':
                filtered_items = [item for item in items if item.get(attribute, '').lower() == value.lower()]

        else:
            if operation == 'eq':
                filtered_items = [item for item in items if item.get(attribute, 0) == value]
            elif operation == 'gt':
                filtered_items = [item for item in items if item.get(attribute, 0) > value]
            elif operation == 'lt':
                filtered_items = [item for item in items if item.get(attribute, 0) < value]
